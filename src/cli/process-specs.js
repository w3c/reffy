 ...
 * to process, `crawl folder` is the name of the folder where the crawl report
...
const {processSpecification, requireFromWorkingDirectory, completeWithAlternativeUrls} = require('../lib/util');
 ...
 */
async function extractWebrefData(spec, crawlOptions) {
...
/**
 * Processes the specifications listed in the given JSON file and generates a
 * crawl report in the given folder.
 *
 * @function
 * @param {String} resultsPath Folder that is to contain the crawl report
 * @param {Object} options Process options
 * @return {Promise<void>} The promise that the processing will have been made
 */
function processSpecs(resultsPath, options) {
    if (!resultsPath) {
    ...
    function prepareListOfSpecs(list) {
        return list
            .map(spec => (typeof spec === 'string') ?
                ...
                spec)
            .filter(spec => !!spec)
            .map(completeWithAlternativeUrls);
    }
    ...
    return crawlSpecs(requestedList, extractWebrefData, options)
...
**************************************************/
module.exports.extractWebrefData = extractWebrefData;
module.exports.processSpecs = processSpecs;
...
if (require.main === module) {
    ...
    processSpecs(resultsPath, crawlOptions)
        .then(_ => {
